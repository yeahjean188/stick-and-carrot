...
async function next() {
    ...
    if (nextCallCount <= 5){
        ...
        try{
            ...
            //dall.e 불러오기
            showLoading2();
            const d2_response = await fetch('https://62x2jqh5bc6s353c7rzm77iq3u0dubtg.lambda-url.ap-northeast-2.on.aws/generate', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
        ...
        try{
            const response3 = await fetch('https://62x2jqh5bc6s353c7rzm77iq3u0dubtg.lambda-url.ap-northeast-2.on.aws/fortuneTell', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            ...
            const d3_data = await d3_response.json();
            console.log(d3_data)
            const imageUrl3 = d3_data.data;
...
function showLoading2(){
    document.querySelector('.loading3').classList.add('show');
}
//로딩 텍스트 비표시
function removeLoading2(){
    document.querySelector('.loading3').classList.remove('show');
}
